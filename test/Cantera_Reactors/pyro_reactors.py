...
from reactor_system import ReactorSystemOde, ReactorSystemPyroOde, state_filter
...
from leap.rk import ODE23MethodBuilder, ODE45MethodBuilder, SSP43MethodBuilder
...
method_fast_species = []
method_fast_rhs = []
method_fast_frac_sum = []
method_fast_num_neg = []
method_fast_neg = []
method_errs = []

odes = []
odes.append(ReactorSystemOde(ar, gas))
odes.append(ReactorSystemPyroOde(ar, gas))
...
for ode in odes:
    ...
    #method = ODE23MethodBuilder("y", rtol=rtol, atol=atol, use_high_order=True,
    #                            state_filter_name='filter')
    #method = SSP43MethodBuilder("y", rtol=rtol, atol=atol,
    #                            use_high_order=True, state_filter_name='filter')
    #method = SSP43MethodBuilder("y", use_high_order=True,
    #                            state_filter_name='filter')
    ...
    #state = np.append(state, np.float128(gas.T))
    state = np.append(state, gas.T)
    state = np.append(state, 1.0)
    #state = np.append(state, np.float128(gas.Y))
    ...
    # The hard part - insert functions from Cantera's kinetics objects
    # for each reactor?
    stepper = stepper_cls(
            function_map={
                "<func>y": lambda t, y: ode(t, y),
                "<func>filter": lambda y: state_filter(y),
                })
    ...
    for i in range(0, len(times)):
        ...
        fast_frac_sum.append(0.0)
        for j in range(0, gas.n_species):
            fast_species[j].append(species_temp[j])
            fast_frac_sum[-1] += values[i][5+j]
    ...
    method_fast_num_neg.append(fast_num_neg)
    method_fast_neg.append(fast_neg)
    method_fast_frac_sum.append(fast_frac_sum)
...
plt.xlabel("t")
plt.ylabel("dt")
plt.legend(['Cantera', 'Pyro'])
plt.tight_layout()
plt.show()

plt.clf()
plt.plot(method_times[0], method_fast_num_neg[0],
         method_times[1], method_fast_num_neg[1])
plt.title("Number of Negative Mass Fractions in Reactor 2")
plt.xlabel("t")
plt.ylabel("No. of Mass Fractions")
plt.tight_layout()
plt.legend(['Cantera', 'Pyro'])
plt.show()

plt.clf()
plt.plot(method_times[0], np.log10(method_fast_neg[0]),
         method_times[1], np.log10(method_fast_neg[1]))
plt.title("Magnitude of Negative Mass Fraction Sum in Reactor 2: Log Scale")
plt.xlabel("t")
plt.ylabel("log10(Sum of Negative Mass Fractions)")
plt.legend(['Cantera', 'Pyro'])
plt.tight_layout()
plt.show()
...
plt.figlegend(h, ['Reactor 1 - Cantera', 'Reactor 2 - Cantera',
                  'Reactor 1 - Pyro', 'Reactor 2 - Pyro'],
                  loc='lower right')
plt.tight_layout()
plt.show()
...
#plt.clf()
#plt.plot(method_times[0], np.log10(np.array(method_errs[0])),
#         method_times[1], np.log10(np.array(method_errs[1])))
#plt.title("Log of Weighted Error Norm Evolution")
#plt.xlabel("t")
#plt.ylabel("E")
#plt.legend(['Cantera', 'Pyro'])
#plt.tight_layout()
...
# Perform a comparison of the integrated states,
# initially using 2-norms.
#state_error_norm = []
#for i in range(0, len(method_times[0])):
#    ct_species = method_fast_species[0]
#    pyro_species = method_fast_species[1]
#    ct_species_temp = []
#    pyro_species_temp = []
#    ct_species_temp.append(method_fast_temps[0][i])
#    ct_species_temp.append(method_fast_vol[0][i])
#    print(i)
#    pyro_species_temp.append(method_fast_temps[1][i])
#    pyro_species_temp.append(method_fast_vol[1][i])
#    for j in range(0, gas.n_species):
#        ct_species_temp.append(ct_species[j][i])
#        pyro_species_temp.append(pyro_species[j][i])
#    ct_norm = np.linalg.norm(np.array(ct_species_temp))
#    pyro_norm = np.linalg.norm(np.array(pyro_species_temp))
#    state_error_norm.append((ct_norm - pyro_norm)/ct_norm)

#plt.clf()
#plt.plot(method_times[0], np.log10(abs(np.array(state_error_norm))))
#plt.title("Log of State Relative Error (2-Norm)")
#plt.xlabel("t")
#plt.ylabel("dt")
#plt.tight_layout()
#plt.show()
...
#    plt.legend(['Cantera', 'Pyro'])
#    plt.tight_layout()
#    plt.savefig("PYRO_COMBO/MASS_FRAC/species_{j}.png".format(j=i))
