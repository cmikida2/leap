"""
ORIGINAL CANTERA TWO-REACTOR EXAMPLE
...
csvfile.writerow(['time (s)','T1 (K)','P1 (Bar)','V1 (m3)',
                  'T2 (K)','P2 (Bar)','V2 (m3)'])
states1 = ct.SolutionArray(ar, extra=['t', 'V', 'm'])
states2 = ct.SolutionArray(gas, extra=['t', 'V', 'm'])

for n in range(n_steps):
    time += 4.e-4
    sim.advance(time)
    states1.append(r1.thermo.state, t=time, V=r1.volume, m=r1.volume*r1.thermo.density)
    states2.append(r2.thermo.state, t=time, V=r2.volume, m=r2.volume*r2.thermo.density)
